...
const promisifyJwtSignFn = util.promisify(jwt.sign);
...
const loginUser = async (req, res) => {
    try {
        ...
        const accessToken = await promisifyJwtSignFn(payload, jwtSecret, { expiresIn: '5m' }); // Set appropriate expiration time
        const refreshToken = await promisifyJwtSignFn(payload, jwtRefreshTokenSecret, { expiresIn: '7d' });
...
const getAccessTokenbyRefreshToken = async (req, res) => {
    try {
        ...
        const decoded =  await promisifyJwtVerifyFn(refreshToken,refreshTokenSecret).catch(err=>{
